...
try {
    switch ($method) {
        case 'GET':
            ...
            if ($action === 'get_one' && isset($_GET['item_id'])) {
            ...
            if ($action === 'get_movements' && isset($_GET['item_id'])) {
                $itemId = (int)$_GET['item_id'];
                $stmt = $pdo->prepare("
                    SELECT *
        ...
        case 'POST':
            if ($action === 'create') {
                ...
                if (!isset($data['nombre'], $data['tipo'], $data['unit'])) {
                    json_response(false, null, 'Faltan campos requeridos', 400);
                }

                $stmt = $pdo->prepare("
                    INSERT INTO inventory_items (nombre, tipo, unit, stock_on_hand, stock_reserved, stock_min, stock_max, active)
                    VALUES (:nombre, :tipo, :unit, 0, 0, 10, 1000, 1)
                ");
                $stmt->execute([
                    ':nombre' => $data['nombre'],
                    ':tipo' => $data['tipo'],
                ...
                json_response(true, ['item_id' => $item_id], 'Ingrediente creado');
            }
            ...
            if ($action === 'delete') {
                ...
                $stmt = $pdo->prepare("UPDATE inventory_items SET active = 0 WHERE item_id = :item_id");
                $stmt->execute([':item_id' => $itemId]);

                json_response(true, null, 'Ítem eliminado');
            }
        ...
        case 'PUT':
            // Legacy PUT (usar POST con action=update en su lugar)
            $data = json_decode(file_get_contents('php://input'), true);
            $itemId = (int)($data['item_id'] ?? 0);
